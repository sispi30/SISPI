...
export interface MenuLeaf { href: string; label?: string; pageTitle?: string; vis?: MenuVis; open?: boolean; icon?: string; visMembers?: string[] }
/** 트리의 상위 한 항목 – href가 있으면 단독 메뉴(하위 없음) */
export interface MenuGroupNode { id: string; label: string; href?: string; items: MenuLeaf[]; pageTitle?: string; vis?: MenuVis; open: boolean; icon?: string; visMembers?: string[] }

/**
 * 메뉴 아이콘 입력값 → 실제 Font Awesome class (커스텀 요청).
 *
 * 관리자는 "fa-home"처럼 아이콘 이름만 입력한다(환경설정 > 메뉴 위젯 화면 예시와 동일한 표기).
...
/** 공개범위 판정에 쓰는 방문자 – id는 「램버 선택」 판정용 (v2.0) */
